...
class SentinelRequester:
    def __init__(self, start_date: str, end_date: str, geojson_path: str, save_path: str, 
    ...
    def get_subarea(self, bbox: BBox) -> SentinelHubRequest:
        return SentinelHubRequest(
            ...
            # resolution= sentinelhub.geo_utils.transform_point(),
            data_folder=tempfile.gettempdir(),
            config=self.config,
        )
    
    def read_file(self) -> gpd.GeoDataFrame:
         filepath = os.path.join(self.geojson_path)
         return (
    ...
    def download_and_save(self, bbox_list, save_filepath: str) -> None:
        ...
        rasterio.merge.merge(
                    opened_tiffs,
                    dtype=np.uint16,
                    dst_path=save_filepath,
                )

    def fetch(self) -> None:
        polygon = self.read_file()
        bbox_list = self.prepare_polygon(polygon)
        self.create_file(self.save_path)
        self.download_and_save(bbox_list, self.save_path)


if __name__ == '__main__':
    requester = SentinelRequester(start_date='2024.08.01', end_date='2024.08.10', sentinel='sentinel_1',
                                  root_path='../sentinel_data/geojson_files/', save_path='../sentinel_data/downloaded_tiff/')
    requester.fetch()